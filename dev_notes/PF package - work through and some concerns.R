...
RRtosst(formula = diagnosis ~ group, data = trial, vac_grp = "vaccinate", con_grp = "control")
...
RRtosst(data = data2, formula =  cbind(sum_y, sum_n) ~ group,
        vac_grp = "vaccinate", con_grp = "control")

# A way that would be more inline with how lm, glm, etc works is something like
# RRtosst(formula = sick ~ group, data = trial, vac_grp = "vaccinate", con_grp = "control")
#Then the function does the counting for me.
#What if we have something like:....
# RRtost(formula = sick ~ group + block(cage), data = trail, vac_grp = "vaccinate", con_grp = "control")
...
Table6
RRstr(cbind(y, n)    ~ tx    + cluster(clus), Table6, vac_grp = "a", con_grp = "b")
...
RRstr(cbind(sick, n) ~ group + cluster(cage), data.frame(wide),   vac_grp = "vaccinate", con_grp = "control") # Crash because one value is 0
RRstr(cbind(sick, n) ~ group + cluster(cage), data.frame(wide),   vac_grp = "control", con_grp = "vaccinate")
...
RRstr(cbind(sick, n) ~ group + cluster(cage), data.frame(wide),   vac_grp = "vaccinate", con_grp = "control")
RRstr(cbind(sick, n) ~ group + cluster(cage), data.frame(wide),   vac_grp = "control", con_grp = "vaccinate")
